...
function display() {
    ...
    while ($grp_row = $grp_results->fetch(PDO::FETCH_ASSOC)) {
        ...
        $grp_id   = $grp_row["id"];
        $grp_href = (getDatabase()
                     ->query('SELECT COUNT(*) FROM groups WHERE parent_id = '.intval($_REQUEST['parent_id']))
                     ->fetchColumn() > 0) ? "grpdev_list.php?parent_id=$grp_id&tripid={$_REQUEST['tripid']}" : "#";
    ...
    if (!empty($_REQUEST["parent_id"])) {
        ...
        $grp_name = getDatabase()
                    ->query('SELECT name FROM groups WHERE id = '.intval($_REQUEST['parent_id']))
                    ->fetchColumn();
