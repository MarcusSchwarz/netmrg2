...
function display() {
	...
	/*** GROUP LIST ***/
    $grp_results = getDatabase()->query('SELECT id, name, comment FROM groups WHERE parent_id = '.intval($_REQUEST['parent_id']));
	...
	while ($grp_row = $grp_results->fetch(PDO::FETCH_ASSOC)) {
        //todo I don't get the COUNT() statement in $grp_href; basically we are checking if we have results for parent_id, while looping over the results for parent_id??
		$grp_id  = $grp_row["id"];
		$grp_href = (getDatabase()->query('SELECT COUNT(*) FROM groups WHERE parent_id = '.intval($_REQUEST['parent_id']))->fetchColumn() > 0) ? "grpdev_list.php?parent_id=$grp_id&tripid={$_REQUEST['tripid']}" : "#";
...
<?php
    ...
    if (!empty($_REQUEST["parent_id"]))	{
        ...
        $grp_name = getDatabase()->query('SELECT name FROM groups WHERE id = '.intval($_REQUEST['parent_id']))->fetchColumn();
        $title = (empty($grp_name)) ? "Monitored Devices" : "Monitored Devices in Group '$grp_name'";

        $dev_results = getDatabase()->query('SELECT devices.name AS name, devices.ip, devices.id, devices.snmp_version, count(snmp.ifIndex) AS interface_count, count(disk.disk_index) AS disk_count, devices.snmp_uptime, devices.disabled, devices.snmp_avoided FROM dev_parents LEFT JOIN devices ON dev_parents.dev_id = devices.id LEFT JOIN snmp_interface_cache snmp ON devices.id = snmp.dev_id LEFT JOIN snmp_disk_cache disk ON devices.id = disk.dev_id WHERE grp_id = '.intval($_REQUEST['parent_id']).' GROUP BY devices.id ORDER BY name ASC');
        $column_results = getDatabase()->query('SELECT DISTINCT dev_props.name AS name FROM dev_parents LEFT JOIN devices ON dev_parents.dev_id = devices.id LEFT JOIN dev_props ON devices.dev_type = dev_props.dev_type_id WHERE grp_id = '.intval($_REQUEST['parent_id']).' ORDER BY dev_props.name ASC');
        ...
        while ($row = $column_results->fetch(PDO::FETCH_ASSOC)) {
        ...
        $count = 0;
        while($dev_row = $dev_results->fetch(PDO::FETCH_ASSOC)) {
            $dev_id  = $dev_row["id"];
            ...
            /* Device Properties */
            $prop_res = getDatabase()->query('SELECT name, value FROM dev_prop_vals vals LEFT JOIN dev_props props ON vals.prop_id = props.id WHERE dev_id = '.$dev_id);

            $props = array();
            while ($proprow = $prop_res->fetch(PDO::FETCH_ASSOC)) {
