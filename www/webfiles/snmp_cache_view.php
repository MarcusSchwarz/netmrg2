...
function make_interface_graph($dev_id, $index) {
	// get snmp index data
    $r_snmp = getDatabase()->query('SELECT * FROM snmp_interface_cache WHERE dev_id = '.intval($dev_id).' AND ifIndex = '.intval($index))->fetch(PDO::FETCH_ASSOC);
...
function make_disk_graph($dev_id, $index) {
	// get snmp index data
    $r_snmp = getDatabase()->query('SELECT * FROM snmp_disk_cache WHERE dev_id = '.intval($dev_id).' AND disk_index = '.intval($index))->fetch(PDO::FETCH_ASSOC);
...
function view_disk_cache() {
    ...
    $rowcount = getDatabase()->query('SELECT COUNT(*) FROM snmp_disk_cache WHERE dev_id = '.intval($_REQUEST['dev_id']))->fetchColumn();
