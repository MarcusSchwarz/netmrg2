...
function end_page() {
    ...
    <div id="footer">
        ...
        <a href="index/about">&copy; 2003-2007</a>
    </div> <!-- end #footer -->
        ...
        foreach ($elements as $curitem) {
            ...
            <td class="<?php echo $style ?>" nowrap="nowrap">
                ...
                if (!empty($curitem["checkboxname"]) && !empty($curitem["checkboxid"])) {
                    ...
                    if (isset($curitem["checkdisabled"]) && $curitem["checkdisabled"]) {
...
function make_edit_table($title, $onsubmit = "")
{
    ...
    ?>
    <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" name="editform"<?php if (!empty($onsubmit)) {
...
function make_edit_submit_button() {
    ...
    <tr>
        <td class="editfield0" align="right">
...
function make_edit_select_subdevice($subdev_id_cur, $prepended_array = array(), $select_options = "") {
    ...
    $subdev_total   = getDatabase()
                      ->query('SELECT COUNT(devices.name) FROM sub_devices LEFT JOIN devices ON sub_devices.dev_id = devices.id')
                      ->fetchColumn();
            ...
            for (i = 0; i < document.<?php echo $prefix; ?>form.elements.length; i++) {
                if ((document.<?php echo $prefix; ?>form.elements[i].type == 'checkbox') && (document.<?php echo $prefix; ?>form.elements[i].disabled == false)) {
                    if (method == 0) {
                        ...
                        document.<?php echo $prefix; ?>form.elements[i].checked = !document.<?php echo $prefix; ?>form.elements[i].checked;
                    }
                }
            }
        }
