...
function dereference_templated_monitor($mon_id, $subdev_id) {
    if ($mon_id > 0) {
        $row    = getDatabase()
                  ->query('SELECT test_id, test_type, test_params FROM monitors WHERE id = '.intval($mon_id))
...
function rrd_watermark() {
    ...
    if (!strstr($GLOBALS['netmrg']['rrdtool_version'], "1.0")) {
        return " -W 'NetMRG - www.netmrg.net' ";
    }
    else {
        return "";
    }
...
function get_graph_command($type, $id, $hist) {
    ...
    switch ($type) {
        case "mon":
            return monitor_graph_command($id, $GLOBALS['TIMEFRAMES'][$hist]);
        case "tinymon":
            return tiny_monitor_graph_command($id, $GLOBALS['TIMEFRAMES'][$hist]);
        case "custom":
            return custom_graph_command($id, $GLOBALS['TIMEFRAMES'][$hist], false, false);
        case "custom_item":
            return custom_graph_command($id, $GLOBALS['TIMEFRAMES'][$hist], false, true);
        case "template":
            return custom_graph_command($id, $GLOBALS['TIMEFRAMES'][$hist], true, false);
        case "template_item":
            return custom_graph_command($id, $GLOBALS['TIMEFRAMES'][$hist], true, true);
    }
...
function custom_graph_command($id, $timeframe, $templated, $single_ds) {
    ...
    if ($single_ds) {
        $ds_id = $id;
        $id    = getDatabase()
                 ->query('SELECT graph_id FROM graph_ds WHERE id = '.intval($id))
                 ->fetchColumn();
    }

    $graph_row = getDatabase()
                 ->query('SELECT * FROM graphs WHERE id = '.intval($id))
                 ->fetch(PDO::FETCH_ASSOC);
    ...
    $padded_length = 5;
    $ds_row        = getDatabase()
                     ->query('SELECT max(length(graph_ds.label)) as maxlen FROM graph_ds WHERE graph_ds.graph_id = '.intval($id).$ew)
                     ->fetchColumn();
    ...
    if ($single_ds) {
        $ds_total   = getDatabase()
                      ->query('SELECT COUNT(*) FROM graph_ds WHERE id = '.intval($ds_id))
                      ->fetchColumn();
        $ds_results = getDatabase()->query('SELECT * FROM graph_ds WHERE id = '.intval($ds_id));
    }
    else {
        $ds_total   = getDatabase()
                      ->query('SELECT COUNT(*) FROM graph_ds WHERE graph_id = '.intval($id).$ew)
                      ->fetchColumn();
