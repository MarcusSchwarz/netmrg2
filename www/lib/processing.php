...
function apply_template($subdev_id, $template_id) {
    ...
    $q_rows = getDatabase()->query('SELECT COUNT(monitors.id) FROM graph_ds, monitors WHERE graph_ds.graph_id = '.intval($template_id).' AND graph_ds.mon_id = monitors.id')->fetchColumn();
	for ($i = 0; $i < $q_rows; $i++) {
    ...
    $sd_row = getDatabase()->query('SELECT dev_id FROM sub_devices WHERE id = '.intval($subdev_id))->fetchColumn();
    $pos_row = getDatabase()->query('SELECT MAX(pos)+1 AS newpos FROM view WHERE object_type = "device" AND object_id = '.intval($sd_row))->fetchColumn();
	if (empty($pos_row)) {
		$pos_row = 1;
	}
    ...
    $pos_row = getDatabase()->query('SELECT MAX(pos)+1 AS newpos FROM view WHERE object_type = "subdevice" AND object_id = '.intval($subdev_id))->fetchColumn();
	if (empty($pos_row)) {
		$pos_row = 1;
	}
...
function get_group_status($grp_id) {
    ...
    $dev_row = getDatabase()->query(' SELECT MAX(devices.status) AS status FROM dev_parents, devices WHERE grp_id = '.intval($grp_id).' AND dev_parents.dev_id = devices.id AND devices.status < 4 GROUP BY grp_id')->fetch(PDO::FETCH_ASSOC);
...
function get_short_monitor_name($mon_id) {
    ...
    $mon_row = getDatabase()->query('SELECT test_id, test_params, test_type FROM monitors WHERE monitors.id = '.intval($mon_id))->fetch(PDO::FETCH_ASSOC);
...
function get_short_property_name($prop_id) {
    ...
    $prop_row = getDatabase()->query('SELECT test_id, test_params, test_type FROM dev_props WHERE dev_props.id = '.intval($prop_id))->fetch(PDO::FETCH_ASSOC);
...
function get_short_test_name($test_type, $test_id, $test_params) {
    ...
    $res = getDatabase()->query($test_query)->fetchColumn();
...
function get_monitor_name($mon_id) {
    ...
    $row = getDatabase()->query('SELECT  devices.name AS dev_name, sub_devices.name AS sub_name FROM monitors LEFT JOIN sub_devices ON monitors.sub_dev_id = sub_devices.id LEFT JOIN devices ON sub_devices.dev_id = devices.id WHERE monitors.id = '.intval($mon_id))->fetch(PDO::FETCH_ASSOC);

	return $row["dev_name"] . " - " . $row["sub_name"] . " (" . get_short_monitor_name($mon_id) . ")";
} // end get_monitor_name()

function get_graph_name($graph_id) {
	return getDatabase()->query('SELECT name FROM graphs WHERE id = '.intval($graph_id))->fetchColumn();
}

function get_group_name($grp_id) {
    return getDatabase()->query('SELECT name FROM groups WHERE id = '.intval($grp_id))->fetchColumn();
}

function get_device_name($dev_id) {
    return getDatabase()->query('SELECT name FROM devices WHERE id = '.intval($dev_id))->fetchColumn();
}

function get_sub_device_name($sub_dev_id) {
	$dev_query = getDatabase()->query('SELECT name FROM sub_devices WHERE id = '.intval($sub_dev_id))->fetchColumn();
	if (empty($dev_query)) {
        return "Not Set";
    }
	return $dev_query;
...
function get_dev_sub_device_name($sub_dev_id) {
    $dev_query = getDatabase()->query('SELECT devices.name AS dev_name, sub_devices.name AS sub_name FROM sub_devices LEFT JOIN devices ON sub_devices.dev_id = devices.id WHERE sub_devices.id = '.intval($sub_dev_id))->fetch(PDO::FETCH_ASSOC);
...
function get_event_name($event_id) {
    return getDatabase()->query('SELECT name FROM events WHERE id = '.intval($event_id))->fetchColumn();
}
...
function GetSubdeviceParent($subdevice_id) {
    return getDatabase()->query('SELECT dev_id FROM sub_devices WHERE id = '.intval($subdevice_id))->fetchColumn();
}
...
function GetUsername($uid) {
    return getDatabase()->query('SELECT user FROM user WHERE id ='.intval($uid))->fetchColumn();
}
...
function move_graph_item($graph_id, $graph_item_id, $direction) {
    ...
    $numrows = getDatabase()->query('SELECT COUNT(id) FROM graph_ds WHERE graph_id = '.intval($graph_id))->fetchColumn();
...
function delete_view_item($item_id) {
	$r = getDatabase()->query('SELECT pos, object_id, object_type FROM view WHERE id = '.intval($item_id))->fetch(PDO::FETCH_ASSOC);
...
function delete_device($device_id, $group_id = false) {
	if ($group_id !== false) {
        ...
        $dev_parent_row = getDatabase()->query('SELECT count(*) AS count FROM dev_parents WHERE dev_id = '.intval($device_id))->fetch(PDO::FETCH_ASSOC);
	}
	else {
...
function delete_ds($ds_id) {
	$r = getDatabase()->query('SELECT graph_id, position FROM graph_ds WHERE id = '.intval($ds_id))->fetch(PDO::FETCH_ASSOC);
