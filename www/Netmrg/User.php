...
class User {
    ...
    protected function load($userid) {
        if (intval($userid) == $userid) {
            ...
            $s = getDatabase()->query('SELECT * FROM user WHERE user = '.getDatabase()->quote($userid));
        }
        if (empty($s)) {
    ...
    public function create(array $patchvars) {
        ...
        if (!empty($args['arguments'])) {
            $query = 'INSERT INTO user SET '.implode(', ', $args['arguments']); // I don't like that style of INSERT INTO, but though it's quite handy
    ...
    public function patch(array $patchvars) {
        ...
        if (!empty($args['arguments'])) {
            $query = 'UPDATE user SET '.implode(', ', $args['arguments']).' WHERE id = '.$this->id;
            getDatabase()->prepare($query)->execute($args['values']);
        }
