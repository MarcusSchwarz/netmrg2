...
class Auth
{
    ...
    {
        return $this->userExistsInDatabase($GLOBALS['netmrg']['defaultMapUser']);
    }
    ...
    public function getUsersPermissionLevel($user = null)
    {
        ...
        if ($this->userIsLoggedIn()) {
            $s = $this->db->prepare('SELECT IF(disabled = 0, permit, :permit) AS permit FROM user WHERE user = :user');
